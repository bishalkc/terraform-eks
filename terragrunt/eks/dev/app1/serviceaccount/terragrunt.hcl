...
#------------------------------------------------------
terraform {
  source = "${get_path_to_repo_root()}//modules/sa-role"
}
...
dependency "secretmanager" {
  config_path = "../secretmanager"

  mock_outputs = {
    secret_manager_arn = ["arn:aws:secretmanager:us-east-1:${get_aws_account_id()}:secretedemo"]
  }
}
