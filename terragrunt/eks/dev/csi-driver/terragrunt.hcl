...
#------------------------------------------------------
terraform {
  source = "${get_path_to_repo_root()}//modules/csi-helm"
}
...
inputs = {
  ...
  eks_cluster_ca_certificate = dependency.eks.outputs.eks_cluster_ca_certificate
  eks_cluster_endpoint       = dependency.eks.outputs.eks_cluster_endpoint

  # SECRETS
  secret_manager_arn = dependency.secretmanager.outputs.database_secret_manager_arn[0]
}

dependency "secretmanager" {
  config_path = "../app/secretmanager"

  mock_outputs = {
    database_secret_manager_arn = ["arn:aws:secretmanager:us-east-1:${get_aws_account_id()}:secretedemo"]
  }
}
